"""add contradiction_note to athlete_memory_facts

Revision ID: 20260709_000001
Revises: 20260705_000001
Create Date: 2026-07-09 00:00:01
"""

from alembic import op
import sqlalchemy as sa
from sqlalchemy import inspect as sa_inspect


revision = "20260709_000001"
down_revision = "20260705_000001"
branch_labels = None
depends_on = None


def upgrade() -> None:
    bind = op.get_bind()
    inspector = sa_inspect(bind)
    existing_columns = {c["name"] for c in inspector.get_columns("athlete_memory_facts")}
    if "contradiction_note" not in existing_columns:
        op.add_column(
            "athlete_memory_facts",
            sa.Column("contradiction_note", sa.Text(), nullable=True),
        )


def downgrade() -> None:
    bind = op.get_bind()
    inspector = sa_inspect(bind)
    existing_columns = {c["name"] for c in inspector.get_columns("athlete_memory_facts")}
    if "contradiction_note" in existing_columns:
        op.drop_column("athlete_memory_facts", "contradiction_note")
